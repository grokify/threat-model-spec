# Threat Model Specification v0.9.0
...
- **Agentic Collective Modeling**: Agent collectives, emergent coordination channels, and reward-hacking dynamics (v0.9.0)

## What's New in v0.9.0

v0.9.0 adds first-class modeling for **agentic collectives** — populations of autonomous agents that coordinate, possibly emergently, toward shared objectives (e.g. an agent swarm that self-organizes over a covert side channel). All new fields are additive and fully backward compatible with v0.8.0.

### Agent Collectives

A new optional top-level `agentCollectives` field on `ThreatModel` describes a population of coordinating agents, including an optional `rewardHacking` block capturing the misaligned-objective dynamic that drove out-of-scope behavior:
...
```json
{
  "agentCollectives": [
    {
      "id": "collective-1",
      "name": "Build-Agent Swarm",
      "population": 200,
      "models": ["model-a", "model-b"],
      "coordinationChannel": "Artifactory directory-name message board",
      "emergent": true,
      "emergentBehaviors": ["shared mailbox convention", "artifact signing scheme", "peer veto norm"],
      "rewardHacking": {
        "objective": "Pass the build grader",
        "intendedPath": "Produce a correct build artifact",
        "unintendedPath": "Encode expected grader output in artifact metadata",
        "impossibleTaskRate": 0.3,
        "description": "A third of assigned builds were unsolvable as specified, incentivizing gaming the grader."
      }
    }
  ]
}
...
### `coordination` Flow Type

The `FlowType` enum gains a `coordination` value for rendering an emergent/covert inter-agent coordination channel distinctly (dashed, distinct teal stroke) in DFD and attack-chain diagrams.

### Built-in Attack Patterns

Two reusable attack-pattern templates are added to the built-in catalog:

- `reward-hacking` — an agent optimizes the measured reward signal instead of the intended objective (specification gaming). Mapped to MITRE ATLAS and OWASP ASI.
- `emergent-agent-coordination` — autonomous agents establish an unintended coordination side channel and self-organize collective behavior. Mapped to MITRE ATLAS and OWASP ASI.
