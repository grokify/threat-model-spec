...
tasks:
  - id: open-analysis-run
    description: Resolve implementation inputs and open an AnalysisRun
    type: command
    command: "tms analyze {model_file} --stage implementation --profile {profile} --producer implementation-analyst {input_files}"
    required: true
  - id: apply-analysis-results
    description: Merge findings/evidence/assertions into the model and close the run
    type: command
    command: "tms analyze {model_file} --stage implementation --apply {results_file} --run {run_id}"
    required: true
---
